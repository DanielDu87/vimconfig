...
- **Explorer 宽度**：`Snacks Explorer` 的侧边栏宽度**严格固定为 30**（无论手动拖拽大于或小于 30 都会自动恢复）。
...
- **持久化记录**：书签保存至 `/Users/dyx/Documents/bookmarks.json`。
...
- **智能操作**：
    ...
    - **宽度固定**：侧边栏宽度**严格固定为 30**，无论手动拖拽大于或小于都会自动恢复。

### 💾 智能保存 (Smart Save)
- **无名 Buffer 自动保存**：编辑 `[No Name]` buffer 时按 `:w` 自动提示输入文件名。
- **另存为功能**：`<leader>fs` 弹出窗口输入新文件名，自动保留原路径和文件名。
- **自动创建目录**：保存时若目录不存在会自动创建。
- **无缝切换**：保存后自动切换到新文件，不保留空白 buffer。
- **多种触发方式**：
    - 命令行：`:w` / `:write`
    - 快捷键：`<C-s>` / `<leader>w`
    - 另存为：`<leader>fs`
...
| `<leader>tv` | **水平终端 (左右分割)** | |
| `<leader>tt` | **标签页终端** | |
| `<C-\>` | **切换/呼出上次使用的终端 (智能)** | 全局 (含终端) |
| `<Esc>` | **退出终端模式返回普通模式** | 终端模式 |

> **终端特性**：所有终端均已移除左边距（禁用 signcolumn、foldcolumn、行号），确保内容最大化显示。
...
### 2026-04-13
- 🔧 **优化 Explorer 宽度策略**：侧边栏宽度**严格固定为 30**，无论手动拖拽大于或小于 30 都会自动恢复
  - 移除了宽度持久化逻辑（不再保存到文件）
  - 启动时、窗口调整时、切换窗口时、甚至光标移动时都会强制检查并修正宽度
  - 多重保障机制确保宽度始终为 30
