...
- **直播级日志体验** (`<leader>rl`)：
  ...
  - **实体识别**：精确高亮 URL、文件路径、`runserver` 命令、命令行参数（如 `-u`）及非零退出状态码。
  - **常规输出纯白化**：确保普通日志文本为纯白色，避免视觉干扰。
- **进程管控**：`<leader>rs` 一键杀死相关联的后台进程及占用的端口。

### 🔧 全能调试中心 (DAP Debugging)
集成了完整的 **Debug Adapter Protocol** 调试套件，支持类似 IDE 的可视化调试体验：
- **多语言支持**：
  - **Python/Django**：支持单文件调试、Django `runserver` 调试以及进程附加。
  - **前端 JS/TS/Vue**：支持 Node.js 环境调试及 Chrome 浏览器远程联动调试。
- **可视化交互**：
  - 自动管理 `dap-ui` 布局，开始调试时自动打开变量、堆栈、断点列表。
  - **代码虚拟文本**：调试过程中在代码行末实时显示变量当前值。
- **自动化适配器**：通过 Mason 自动安装和更新 `debugpy` 与 `js-debug-adapter`。
...
| 快捷键 | 功能 | 说明 |
| :--- | :--- | :--- |
| `<leader>db` | **切换断点** | 在当前行设置/取消断点 |
| `<leader>dc` | **开始/继续** | 启动调试或运行到下一个断点 |
| `<leader>di` | **步入 (Into)** | 进入函数内部 |
| `<leader>do` | **步过 (Over)** | 跳到下一行代码 |
| `<leader>du` | **步出 (Out)** | 跳出当前函数 |
| `<leader>dt` | **调试面板** | 强制切换 DAP UI 显示状态 |
| `<leader>dp` | **性能分析** | LazyVim 性能分析工具 |
