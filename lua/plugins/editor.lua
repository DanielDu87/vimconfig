		...
		opts = {
			...
			replace = {
				desc = {
					...
					{ "Quit All", "退出所有窗口" },
					-- Git
					{ "Git Diff", "Git 差异 (Diff)" },
					{ "Git Status", "Git 状态 (Status)" },
					{ "Git Stash", "Git 暂存 (Stash)" },
					{ "Git Blame Line", "显示行变更 (Blame)" },
					{ "Lazygit", "LazyGit 客户端" },
					{ "Commits", "提交记录" },
					{ "Status", "状态" },
					-- LSP/诊断
					...
					{ "Goto References", "查看引用" },
					{ "Goto Implementation", "查看实现" },
					...
					{ "Toggle Diagnostics", "切换诊断显示" },
					{ "Toggle Conceal", "切换隐藏字符" },
					{ "Toggle Spell", "切换拼写检查" },
					{ "Toggle Background", "切换背景深浅" },
					{ "Toggle Pin", "固定缓冲区" },
					{ "Color", "颜色" },
